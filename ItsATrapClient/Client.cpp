...
void Client::sendStateUpdate(int id, struct objectProperty prop)
{
	struct singleStateUpdatePacket p;
	p.eventId = SINGLE_STATE_UPDATE_EVENT;
	p.playerId = playerId;
	p.entry.objectId = id;
	memcpy(&p.entry.prop, &prop, sizeof(struct objectProperty));
...
void Client::requestToSpawnNewObject(struct dynamicObject)
{
