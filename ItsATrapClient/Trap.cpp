...
namespace sg {
	...
	void Trap::draw(glm::mat4 parent, glm::mat4 cam) {
		...
		glm::mat4 new_model = glm::translate(this->getPosition()) * glm::scale(glm::vec3(polyTrapScaleX, polyTrapScaleY, polyTrapScaleZ)) * Utilities::rotateY(rotationAngle + 180.0f) * Utilities::rotateX(rotationAngleX) * Utilities::rotateZ(rotationAngleZ);
	...
	struct trapObject Trap::getTrapObjectForNetworking() {
		...
		t.rotationAngleX = rotationAngleX;
		t.rotationAngleZ = rotationAngleZ;
		return t;
	}
}