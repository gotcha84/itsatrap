...
namespace sg {
	...
	void Building::initializeHeightMap() {
		...
		for (int l = 0; l < 46; l++) {
			...
			for (int i = 0; i < m_nVertices[l]-m_nVertices[l]%12; i+=12) { 
				...
				//if (m_vertices[l][i] < tmpminx) {
				//	tmpminx = m_vertices[l][i];
				//}
				//if (m_vertices[l][i+1] < tmpminy) {
				//	tmpminy = m_vertices[l][i+1];
				//}
				//if (m_vertices[l][i+2] < tmpminz) {
				//	tmpminz = m_vertices[l][i+2];
				//}
				//if (m_vertices[l][i] > tmpmaxx) {
				//	tmpmaxx = m_vertices[l][i];
				//}
				//if (m_vertices[l][i+1] > tmpmaxy) {
				//	tmpmaxy = m_vertices[l][i+1];
				//}
				//if (m_vertices[l][i+2] > tmpmaxz) {
				//	tmpmaxz = m_vertices[l][i+2];
				//}
		

				//tmpminx = min(min(m_vertices[l][i], m_vertices[l][i+3]),  m_vertices[l][i+6]);
				//tmpmaxx = max(max(m_vertices[l][i], m_vertices[l][i+3]),  m_vertices[l][i+6]);
				//tmpminy = min(min(m_vertices[l][i+1], m_vertices[l][i+4]),  m_vertices[l][i+7]);
				//tmpmaxy = max(max(m_vertices[l][i+1], m_vertices[l][i+4]),  m_vertices[l][i+7]);
				//tmpminz = min(min(m_vertices[l][i+2], m_vertices[l][i+5]),  m_vertices[l][i+8]);
				//tmpmaxz = max(max(m_vertices[l][i+2], m_vertices[l][i+5]),  m_vertices[l][i+8]);
	...
	*/
	...
	void Building::loadData(string inputfile) {
		std::vector<tinyobj::shape_t> shapes;
